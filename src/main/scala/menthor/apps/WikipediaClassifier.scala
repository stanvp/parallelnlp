...
object WikipediaClassifier extends TicToc {
  ...
  def main(args: Array[String]) {
    if (args.size < 8) {
      println("Please specify [algorithm] [traning mode] [wikipedia train corpus file] [wikipedia test corpus path] [features file] [evaluation] [benchmark result file] [benchmark iteration]")
